...
import { RequestStatusType, setStatusAC, SetStatusACType, SetErrorACType } from "app/app-reducer"
...
export const tasksReducer = (state = initialState, action: TasksActionsType): TasksStateType => {
  switch (action.type) {
    ...
    case "ADD-TASK":
      return {
        ...state,
        [action.todolistId]: [{ ...action.task, entityTaskStatus: "idle" }, ...state[action.todolistId]],
      }
    case "UPDATE-TASK":
      return {
        ...state,
